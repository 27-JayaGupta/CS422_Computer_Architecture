...
#define DEFAULT_REG_VALUE 10000

// Bypass Sources
#define BYPASS_NONE 0x01
#define BYPASS_EX_EX 0x02
#define BYPASS_MEM_EX 0x04
...
void PIPELINE_REGS::reset_regs() {
   ...
   _opResultHi = 0;
   _opResultLo = 0;       // Result of operation
   _memControl = FALSE;                    // Memory instruction?
   _writeREG = FALSE;
   _writeFREG = FALSE;          // WB control
   _branchOffset = 0;
   _hiWPort = FALSE;
   _loWPort = FALSE;             // WB control
...
public:
   ...
   void Dec (unsigned int ins, Bool actual);			// Decoder function
   ...
   PIPELINE_REGS IF_ID_C, ID_EX_C, EX_MEM_C, MEM_WB_C;
