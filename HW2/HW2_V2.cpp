...
public:
    ...
    inline void predict(bool isForwardBranch) {
        lastPrediction = (!isForwardBranch);
        // return lastPrediction;
    }

    inline void update(BOOL actualPrediction, bool isForwardBranch) {
...
public:
    ...
    inline void predict(UINT32 pc) {
        lastIdxChosen = pc & 0x1FF; // last 9 bits of pc to index into PHT
        lastPrediction = (pht[lastIdxChosen] >= 2);
        // return lastPrediction;
    }
    ...
    inline void update(BOOL actualPrediction, bool isForwardBranch) {
        pht[lastIdxChosen] =  truth_table_2_bit[pht[lastIdxChosen]][actualPrediction];
...
private:
    vector<int> bht;
    vector<int> pht; // 2 bit counter
    int lastBHTIdxChosen = 0;
    int lastPHTIdxChosen = 0;
    bool lastPrediction = false;
...
public:
    ...
    inline bool predict(UINT32 pc) {
        lastBHTIdxChosen = pc & 0x3FF; // last 10 bits of pc to index into PHT
        lastPHTIdxChosen = bht[lastBHTIdxChosen];
        lastPrediction = (pht[lastPHTIdxChosen] >= 2);
        return lastPrediction;
    }
    ...
    inline void update(BOOL actualPrediction, bool isForwardBranch) {
        ...
        // update PHT
        pht[lastPHTIdxChosen] = truth_table_2_bit[pht[lastPHTIdxChosen]][actualPrediction];
...
public:
    ...
    inline void update(BOOL actualPrediction, bool isForwardBranch) {
        ...
        // update PHT
        pht[lastPHTIdxChosen] = truth_table_3_bit[pht[lastPHTIdxChosen]][actualPrediction];
...
public:
    ...
    inline bool predict(void) {
        lastPHTIdxChosen = ghr;
        lastPrediction = (pht[lastPHTIdxChosen] >= 4);
        return lastPrediction;
    }
};
...
private:
    ...
    int lastIdxChosen = 0;
    bool lastPrediction = false;
    bool GAgPrediction = false, SAgPrediction = false;
...
public:
    ...
    inline void predict(UINT32 ghr, BOOL SAgPred, BOOL GAgPred) {
        GAgPrediction = GAgPred;
        SAgPrediction = SAgPred;

        lastIdxChosen = ghr;
        lastPrediction = (metaPredictor[lastIdxChosen] >= 2) ? GAgPred : SAgPred;
        // return lastPrediction;  
    }
...
public:
    ...
    inline void predict(BOOL first, BOOL second, BOOL third) {
        lastPrediction = (first && second) || (first && third) || (second && third);
        // return lastPrediction;
    }
...
private:
    ...
    bool SAgPrediction  = false, GAgPrediction = false, gsharePrediction = false;
    int lastIdxChosen = 0;
    bool lastPrediction = false;
...
public:
    ...
    inline void predict(UINT32 ghr, BOOL SAgPred, BOOL GAgPred, BOOL gsharePred) {
        GAgPrediction = GAgPred;
        SAgPrediction = GAgPred;
        gsharePrediction = gsharePred;

        lastIdxChosen = ghr;
        bool isGAg = (SAg_GAg[lastIdxChosen] >= 2);
        bool isgshare = isGAg ? (GAg_gshare[lastIdxChosen] >= 2) : (SAg_gshare[lastIdxChosen] >= 2);
        lastPrediction = isgshare ? gsharePred : (isGAg ? GAgPred : SAgPred);
        // return lastPrediction;  
    }
    ...
    inline void update(BOOL actualPrediction, bool isForwardBranch) {
        ...
        if(GAgPrediction != SAgPrediction) {
            bool favor = (GAgPrediction == actualPrediction);
            SAg_GAg[lastIdxChosen] = truth_table_2_bit[SAg_GAg[lastIdxChosen]][favor];
        }

        if(GAgPrediction != gsharePrediction) {
            bool favor = (gsharePrediction == actualPrediction);
            GAg_gshare[lastIdxChosen] = truth_table_2_bit[GAg_gshare[lastIdxChosen]][favor];
        }

        if(SAgPrediction != gsharePrediction) {
            bool favor = (gsharePrediction == actualPrediction);
            SAg_gshare[lastIdxChosen] = truth_table_2_bit[SAg_gshare[lastIdxChosen]][favor];
        }
...
VOID ConditionalBranchAnalysis(UINT32 pc, bool branch_taken, UINT32 branchTarget) {
    ...
    // prediction
    __fnbt.predict(isForwardBranch);
    __bim.predict(pc);
    bool sag_pred = __sag.predict(pc);
    bool gag_pred = __gag.predict();
    bool gshare_pred = __gshare.predict(pc);
    __sag_gag_hybrid.predict(ghr, sag_pred, gag_pred);
    __maj.predict(sag_pred, gag_pred, gshare_pred);
    __tournamentMP.predict(ghr, sag_pred, gag_pred, gshare_pred);

    // updation
    __fnbt.update(branch_taken, isForwardBranch);
    __bim.update(branch_taken, isForwardBranch);
    __sag.update(branch_taken, isForwardBranch);
    __gag.update(branch_taken, isForwardBranch);
    __gshare.update(branch_taken, isForwardBranch);
    __sag_gag_hybrid.update(branch_taken, isForwardBranch);
    __maj.update(branch_taken, isForwardBranch);
    __tournamentMP.update(branch_taken, isForwardBranch);
}
